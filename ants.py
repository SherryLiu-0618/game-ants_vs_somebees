            ...
            if self.ant is None:
                ...
                if self.ant.is_container == True:
                    if self.ant.can_contain(insect) == True:
                        if insect.is_container == True:
                            assert self.ant is None, 'Two ants in {0}'.format(self)
                        else:
                            self.ant.contain_ant(insect)
                    elif self.ant.can_contain(insect) == False:
                        assert self.ant is None, 'Two ants in {0}'.format(self)
                else:
                    if insect.is_container == True:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.name == "Queen":
                if insect.is_placed == True:
                    if insect.fake == False:
                        self.ant = insect
                        exit()
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        newplace = self.place
        count_steps = 0
        while newplace is not hive:
            if newplace.bees != [] and count_steps >= self.min_range and count_steps <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for every_bee in all_bees:
                every_bee.reduce_armor(self.damage + amount)
        else:
            for every_bee in all_bees:
        ...
        if self.fake == False:
            ...
            while position != None:
                if position.ant != None:
                    if position.ant not in self.ants_counted:
                        position.ant.damage *= 2
                        if position.ant.is_container == True:
                            if position.ant.contained_ant != None:
                                if position.ant.contained_ant.name != "Fire":
                                    position.ant.contained_ant.damage *= 2
                                    self.ants_counted.append(position.ant.contained_ant)
                        self.ants_counted.append(position.ant)
                    elif position.ant in self.ants_counted:
                        if position.ant.is_container == True:
                            if position.ant.contained_ant != None:
                                if position.ant.contained_ant not in self.ants_counted:
                                    position.ant.contained_ant.damage *= 2
                                    self.ants_counted.append(position.ant.contained_ant)
